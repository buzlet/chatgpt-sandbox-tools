...
# Cloudflare Browser Integrity / bot checks may reject Python's default
# Python-urllib user agent before the request ever reaches the Worker.
# The production path is ChatGPT's HTTPS web gateway, so the self-test uses
# the same recognizable client class instead of testing an unrelated UA.
CLIENT_HEADERS = {
...
def read_json(url, opener=None, attempts=10):
    ...
    for attempt in range(attempts):
        ...
        try:
            ...
            # Temporary workers.dev deployment can briefly return edge-level
            # 404/non-JSON while propagation finishes. 403 is deliberately not
            # retried because it may be our application authorization check.
...
status, _, _, response = relay(
    "GET", "https://httpbun.com/bearer/cloudflare-relay-selftest", rid_auth,
